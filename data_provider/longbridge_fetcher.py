...
class LongbridgeFetcher(BaseFetcher):
    ...
    def get_main_indices(self, region: str = "cn") -> Optional[list[Dict[str, Any]]]:
        """Fetch US major indices from Longbridge, fallback to later providers if incomplete."""
        if region != "us":
            return None
        if not self.is_available_for_request("main_indices"):
            return None
        ...
        for code, symbol, name in indices:
            try:
                ...
                open_price = safe_float(getattr(q, "open", None), price) or price
                high = safe_float(getattr(q, "high", None), price) or price
                low = safe_float(getattr(q, "low", None), price) or price
                volume = int(getattr(q, "volume", 0) or 0)
                turnover = safe_float(getattr(q, "turnover", None), 0.0) or 0.0

                change = round(price - prev_close, 4)
                change_pct = round(change / prev_close * 100, 2)
                amplitude = round((high - low) / prev_close * 100, 2) if high and low else 0.0
                ...
                logger.info("[Longbridge] 获取美股指数 %s(%s) 失败: %s", name, symbol, e)
                if self._is_connection_error(e):
                    self._mark_connection_cooldown(e)

        if len(results) >= 3:
            logger.info("[Longbridge] 成功获取 %d 个美股指数行情", len(results))
            return results

        logger.warning("[Longbridge] 美股指数行情不足，回退到后续数据源")
        return None
    # ------------------------------------------------------------------
